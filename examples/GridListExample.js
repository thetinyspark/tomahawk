...
Main.prototype._completeHandler = function(event)
{
	...
	var prop = null;
	var list = new tomahawk_ns.GridList();
	...
	list.elementsPerPage = 8;
	list.elementsPerRow = 4;
	list.elementWidth = 80;
	list.elementHeight = 80;
	list.margin = 10;
	list.refresh(0);
	
	document.getElementById("prevBtn").addEventListener("click", list.prev.bind(list));
	document.getElementById("nextBtn").addEventListener("click", list.next.bind(list));
};
