window.onload = function()
{
	...
	var list = [test_addCharAt,test_removeCharAt,test_addTextAt,
				...
				test_mat_transformPoint,test_assetsManager,test_stage_multiton, test_smartbind, test_stage_destroy,
				test_to_json,test_to_flat_bmp];
...
function test_to_json()
{
	...
	return (obj != null && obj.x != undefined);
}

function test_stage_destroy()
{
	var stage = tomahawk_ns.Stage.getInstance("test");
	var canvas = document.createElement("canvas")
	var listeners = null;
	...
	listeners = stage.getEventListeners();
	
	return listeners.length == 0 && stage.getCanvas() == null && stage.getContext() == null;
}

function test_stage_multiton()
{
	var stage1 = tomahawk_ns.Stage.getInstance("toto");
	var stage2 = tomahawk_ns.Stage.getInstance("toto");
	var stage3 = null;
	tomahawk_ns.Stage.removeInstance("toto");
	
	stage3 = tomahawk_ns.Stage.getInstance("toto");
...
function test_smartbind()
{
	var bounded1 = test_smartbind.smartBind(this);
	var bounded2 = test_smartbind.smartBind(this);
	var bounded3 = null;
	
	test_smartbind.removeSmartBind(this);
	
	bounded3 = test_smartbind.smartBind(this);
	
	return ( bounded1 == bounded2 && bounded2 != bounded3 );
}
...
{
	var spr = new tomahawk_ns.Sprite();
	var shape1 = new tomahawk_ns.Sprite();
	var shape2 = new tomahawk_ns.Sprite();
	var shape3 = new tomahawk_ns.Sprite();
	var shape4 = new tomahawk_ns.Sprite();
	...
	spr.setChildIndex(shape4,0);
	spr.addChild(shape1);
	
	return ( spr.getChildIndex(shape1) == 3 && spr.getChildIndex(shape4) == 0);
}
